...
export function createIndexes(): void {
  ...
  db.exec('CREATE INDEX IF NOT EXISTS idx_offers_start_date ON offers(start_date)');
  db.exec('CREATE INDEX IF NOT EXISTS idx_offers_created_by ON offers(created_by)');
  ...
  db.exec('CREATE INDEX IF NOT EXISTS idx_requests_offer_id ON requests(offer_id)');
  
  // Leave balances indexes
  db.exec('CREATE INDEX IF NOT EXISTS idx_leave_balances_user_id ON leave_balances(user_id)');
